...
export default function ImageQuiz() {
  ...
  const hasEnough = eligible.length >= QUIZ_CONSTANTS.NUM_OPTIONS;
  ...
  const { speak } = useTTS();
  const { logAnswer, updateWordProgress, saveQuizSession, sessionId } = useTracking('imageQuiz');
  ...
  useEffect(() => {
    ...
    const count = questionCount === 'infinite'
      ? eligible.length
      : Math.min(questionCount, eligible.length);
    ...
    if (index + 1 >= questions.length) {
      ...
      (async () => {
        if (accuracy === 100) {
          ...
          if (success) {
            // 신기록인지 확인하여 UI 피드백
            const isNew = await isNewRecord('imageQuiz', totalTimeMs, accuracy, questionCount || 'infinite');
            if (isNew) {
      ...
      <div className="app-container">
        <div className="app-main">
          ...
          <div className="question-area">
            <h2 className="question-text">🖼️ 그림 보고 맞추기</h2>
            <p>이미지 URL이 있는 단어가 최소 {QUIZ_CONSTANTS.NUM_OPTIONS}개 필요합니다.</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="app-container" style={{ justifyContent: 'center', alignItems: 'center' }}>
        <div className="card" style={{ textAlign: 'center', maxWidth: 500 }}>
          <h2 className="card-title">🖼️ 그림 보고 맞추기</h2>
          <p className="card-subtitle" style={{ marginBottom: 24 }}>그림을 보고 단어를 맞추세요</p>
              ...
              <div style={{ textAlign: 'center', margin: 16 }}>
                {currentQuestion.imageUrl ? (
                  <img
                    ...
                    style={{ maxWidth: 360, maxHeight: 240, borderRadius: 12, objectFit: 'cover' }}
                  />
                ) : (
